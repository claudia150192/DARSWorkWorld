...
#Version 3:  This file is based on workbook 29.  There are many changes yet to be made.  Many items hardwired so progress in other areas could be made.
...
#4) Many question answers are currency and these values are used over and over.  Can we create a currency input type?  if not what is the best way to refer to these values without 
#   continually testing for nil values
#5) How do we reference calculations in other calculations, display rules for questions and display rules for numeric results
#    see State question display rule   #answer = 20 # WebCalc.find_by_name('RAge')
#6) The start dates in the seed file need to be more precise, allow entry of a real date.  Most of these are Jan 1 2010 start
#7) Also need to get correct rate record based on sit date, not always the first record.  I'm guessing this is an aditional calc and not the current "RateSgaDisabledAmt = FederalRate.first.sga_disabled_amount"
#8) Bob wonders why he can't say 'return false' within a webCalc.  false by itself works and return true works as well line 47xx @R_ActualSsiGreaterThanCalcSsiNoOverUnderAdjust 
#9) if you type goofy for the birthdate year the program blows up, does not show the proper validation message  Also unsure how to test validation get bad validation message: See below

#11 Noticed on the deeming page when only the VA_CaidParDeemAmt question showed, tabbing off an illegal entry did not bring up the red rectangle but clicking the accept button did bring up the 
# "there were errors that kept you from proceeding" did appear.   BOB thinks that validation rules are performed even if question is not displayed and may need to account for this.
#12 Noticed on the PMV page if the "PMV Rule: How much In-Kind Support are you considered by SSA to receive?  (Default is the Presumed Maximum Value or PMV.)" questoin is answered incorrectly and the continue button pressed
#  all the answers on the page are changed unexpectedly
# FEATURE request Would like to be able to imbed ansers to question with in question text for later questions See Q91 for an example "How much of your $2000 of unearned income is infrequent or irregular?" where $2000 is the answer to the UnEarnAmt question
#          And validation text  And default ansewr individualized threshold
#13 Notice that if only 1 currency question present on a page [unearned income] and a valid number is entered and enter is pressed a new screen is presented even though more 
#questions should have appeared on unearned income page. These questons do appear if tab is pressed
#14 notice when On Unearned income screen when non-float text is entered and tab key pressed on last field on page error occurs (eval):7:in `Float': invalid value for Float(): "ArgumentError in Situations#jwoww 
#15 It would be good if focus could return to the data entry field after the red box illuminates it because of an error
#16 need to be able to calculate a default value, for example VA_AuxGrantFacilityRateAmt  
...
VA_LiveInAuxGrantFacility = manager.get_answer_by_question_name('VA_LiveInAuxGrantFacility')
VA_CaidLiveInFacility = manager.get_answer_by_question_name('VA_CaidLiveInFacility')
(SsiGet == 'Yes') && (SsiResourceLimitExceed != 'Yes') && (SsiCoupleResourceLimitExceed != 'Yes') && (LiveWithSpouseAndOthers == 'Yes' || LiveWithOthers == 'Yes') && (VA_LiveWithSpouseAuxGrantFacility != 'Yes') && (VA_CaidLiveWithSpouseInFacility != 'Yes') && (VA_LiveInAuxGrantFacility != 'Yes') && (VA_CaidLiveInFacility != 'Yes') 
"
@InKind.save!
...
SsiSpouseDeem = manager.get_answer_by_question_name('SsiSpouseDeem')
VA_CaidSpouseDeem = manager.get_answer_by_question_name('VA_CaidSpouseDeem')
SpouseStudent == 'Yes' && (SsiSpouseDeem  == 'Yes' || VA_CaidSpouseDeem == 'Yes')
"
...
WebNumericalResult.create!(:name => 'RSgaAmtBasedOnDate', :display_rules => 'always', :order => 9, :web_calc_id => @RSgaAmtBasedOnDate.id,    :group_id => @SSI_SSDIWorkIncentives.id)
