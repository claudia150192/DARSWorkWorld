class UpdateQuestionInKindPmvAmtForValidation < ActiveRecord::Migration
  def self.up
    require 'wwweb_session_manager'  
         #26	86	PMV Rule: How much In-Kind Support are you considered by SSA to receive?  (Default is the Presumed Maximum Value or PMV.)	I
         #nKindPMVAmt			IF SsiGet = Y AND SpouseSsiGet = Y, THEN Zero or Positive Number <= RatePmvCoupleAmt, ELSE <= RatePmvAmt	
         #You can not enter an amount higher than the Presumed Maximum Value (PMV) of RateTwoThirdsFbrAmt for an individual or RatePmvCoupleAmt for a couple on SSI.  Enter positive numbers only.  Do not enter $.	EXPRESSION:  IF SpouseSsiGet = Y, THEN RatePmvCoupleAmt, ELSE RatePmvAmt	If InKindThird = N	no		This amount will be added to the user's unearned income. Allowable answer includes Couple PMV if user has spouse because that PMV could be used in spousal deeming.	91.00
        ...
        # calc to insert
         ...
         if @Insert1.blank? then
            @Insert1  = QuestionValidationInsert.new(:question_id => @InKindPMVAmt.id, :insert_number => '1', :webcalc_id_to_insert => @RateTwoThirdsFbrAmt.id, :insert_marker => "~", :insert_type => 'currency')
            @Insert1.save!
        else
          @Insert1.update_attribute(:question_id, @InKindPMVAmt.id)
          @Insert1.update_attribute(:insert_number, '1')
          @Insert1.update_attribute(:webcalc_id_to_insert, @RateTwoThirdsFbrAmt.id)
